...
const resposneBody = <T>(response: AxiosResponse<T>) => response.data;
  ...
  (error: AxiosError) => {
    ...
    switch (status) {
      case 400:
        ...
        if (data.errors) {
          const modalStateErrros = [];
          for (const key in data.errors) {
            if (data.errors[key]) modalStateErrros.push(data.errors[key]);
          }

          throw modalStateErrros.flat();
        } else {
          toast.error(data);
        }
        break;
...
const requests = {
  ...
  post: <T>(url: string, body: {}) =>
    axios.post<T>(url, body).then(resposneBody),
...
const Account = {
  cuurent: () => requests.get<User>("/account"),
